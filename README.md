...
header-only C++ stereo VO library — in a ROS 2 node, simulates a TurtleBot4
with stereo cameras in the Gazebo warehouse world, drives it through Nav2
waypoints, and evaluates trajectory accuracy with evo APE.
...
| **Navigation** | Nav2 `BasicNavigator` (`run_waypoints.py`) |
...
| Nav2 | `ros-jazzy-nav2-simple-commander` |
...
### 2 — Drive a waypoint loop

In a second terminal (after `source install/setup.bash`):
...
| `/left/camera_info` | `sensor_msgs/CameraInfo` | Latched once; provides `fx/fy/cx/cy` |
...
| Topic | Type | Notes |
|-------|------|-------|
| `/vo/odometry` | `nav_msgs/Odometry` | Pose in `odom` frame, used by Nav2 |
...
| `base_frame` | `base_footprint` | Child TF frame |
...
| `baseline_m` | `0.075` | Baseline override (m); used when `P[3]≈0` in Gazebo |
...
| **Local BA** | `Estimator` | ≤ 3 KFs, ≤ 100 landmarks, every 2 keyframes, backed out on RMSE increase |
...
| `scripts/run_waypoints.py` | Send a configurable waypoint sequence to Nav2 `BasicNavigator`. |
